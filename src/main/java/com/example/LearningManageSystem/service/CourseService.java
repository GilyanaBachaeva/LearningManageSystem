...
public class CourseService {
    ...
    public CourseDTO addCourse(CourseDTO courseDTO) {
        log.info("Adding group: {}", courseDTO);
    ...
    public CourseDTO createCourseWithSchedule(CourseDTO courseDTO, ScheduleDTO scheduleDTO) {
        log.info("Creating course with name: {}", courseDTO.getName());
        ...
        log.info("Schedule created successfully for course id: {} with schedule id: {}", savedCourse.getId(), savedSchedule.getId());

        courseDTO.setId(savedCourse.getId());
        courseDTO.setScheduleIds(Set.of(savedSchedule.getId()));

        log.info("Returning CourseDTO with id: {} and schedule id: {}", courseDTO.getId(), courseDTO.getScheduleIds());
        return courseDTO;
    }
    ...
    public void deleteCourse(Long id) {
        ...
        if (!courseRepository.existsById(id)) {
            log.error("Course with ID {} not found for deleting", id);
    ...
    public CourseDTO updateCourse(Long id, CourseDTO courseDTO) {
        ...
        CourseEntity course = courseRepository.findById(id)
                .orElseThrow(() -> {
                    log.error("Course with ID {} not found for updating", id);
